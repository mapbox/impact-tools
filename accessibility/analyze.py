...
parser = argparse.ArgumentParser(description="Run zonal stats on area defined by set of isochrone polygons")
parser.add_argument("input", help="geojson file containing polygon collection")
parser.add_argument("--pop_tiff", required=True, help="path to population GeoTIFF")
parser.add_argument("--buffer_distance", type=int, default=1, help="Buffer geometries to incorporate walking time to roads and fix invalid isochrones")
parser.add_argument("--limit", default=None, type=int, help="If provided, load only some features for testing")
parser.add_argument("--no_outputs", default=False, action="store_true", help="Don't save results to a file")
parser.add_argument("--output", required=not '--no_outputs' in sys.argv, help="file path to save dissolved polygons")
parser.add_argument("--points", default=None, help="geojson or csv (csv must have named lat/lon columns)")
parser.add_argument("--points_buffer_distance", type=int, default=4000, help="buffer point geometries to account for walking")
...
def analyze(args):
    ...
    isochrones_gdf = gpd.read_file(args.input)[:args.limit] if args.limit else gpd.read_file(args.input)

    isochrones_gdf = set_utm_from_gdf(isochrones_gdf)

    print(f'Buffering geometry by {args.buffer_distance} meters')
    isochrones_gdf.geometry = isochrones_gdf.geometry.buffer(args.buffer_distance)

    if args.points:
        points = args.points
        print("Reading input points "+points)
        is_json = "json" in Path(points).suffix or "geojson" in Path(points).suffix
        ...
        if is_csv:
            df = pd.read_csv(points)[:args.limit] if args.limit else pd.read_csv(points)
            ...
            points_gdf = gpd.read_file(points)[:args.limit] if args.limit else gpd.read_file(points)
        ...
        print(f'Buffering point gemetry by {args.points_buffer_distance} meters')
        points_buffer_gdf.geometry = points_buffer_gdf.buffer(args.points_buffer_distance)
    ...
    if not args.no_outputs:
        output = args.output
        is_json = "json" in Path(output).suffix or "geojson" in Path(output).suffix
